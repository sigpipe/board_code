...
char *parse_token(char *tok, int maxlen); // parses a space-delimited token
char *parse_spc_delim_token(char *tok, int maxlen); // parses a space-delimited token
...
int   parse_int64(long long *n_p); // skip space, parse dec or hex int. ret non-0 on err
                           // if there's a error it does not change *n_p
int   parse_double(double *d_p); // skip space, parse a double. ret non-0 on err

int parse_double_or_hex(double *d_p);
...
char *parse_get_ptr(void); // returns rest of line to be parsed
char *parse_get_line(void); // returns line currently being parsed
